pipeline {
    ...
    environment {
        ...
        def CONNECT = 'ssh -o StrictHostKeyChecking=no ubuntu@35.182.195.181'
    }
    stages {
        ...
        stage('Deploy') {
            steps {
                echo 'Deploying app'
                sshagent(['server-key']) {
                    sh 'scp -o StrictHostKeyChecking=no -i $SSH_CRED webapp.zip ubuntu@35.182.195.181:/home/ubuntu'
